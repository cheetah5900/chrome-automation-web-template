...
class FlowClient:
    ...
    async def generate_video(self, start_image_media_id: Optional[str], prompt: str,
        ...
        # 1. Auto-upload image if image_path is supplied and start_image_media_id is missing
        if not start_image_media_id and image_path:
            ...
            if os.path.isfile(clean_path):
                ...
                try:
                    ...
                    mime = mimetypes.guess_type(clean_path)[0] or "image/jpeg"
                    upload_res = await self.upload_image(
                        b64, mime_type=mime, project_id=pid, file_name=os.path.basename(clean_path)
                    )
                    start_image_media_id = upload_res.get("_mediaId") or upload_res.get("data", {}).get("media", {}).get("name")
                    logger.info("Auto-uploaded image %s -> mediaId: %s", clean_path, start_image_media_id)
                except Exception as up_err:
                    logger.warning("Failed to auto-upload image %s: %s", clean_path, up_err)
