...
async def create_request(req_type: str, orientation: str = None,
                         ...
                         source_media_id: str = None, edit_prompt: str = None, **_kw) -> dict:
    db = await get_db()
    rid, now = _uuid(), _now()
    async with _db_lock:
        await db.execute(
            """INSERT INTO request (id,project_id,video_id,scene_id,character_id,type,orientation,source_media_id,edit_prompt,created_at,updated_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (rid, project_id, video_id, scene_id, character_id, req_type, orientation, source_media_id, edit_prompt, now, now))
        await db.commit()
